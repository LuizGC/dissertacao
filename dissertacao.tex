...
\resumo{As redes veiculares \emph{ad hoc} (\emph{Vehicular Ad Hoc Networks} - VANETs) são redes com alta mobilidade onde veículos podem comunicar um com os outros ou com infra-estrutura nas vias, como também podem transportar dados fisicamente de um ponto ao outro. Como existe uma grande mobilidade dos veículos em uma cidade, a rede veicular precisa de uma capacidade de adaptação rápida ao contexto. Os agentes de software encaixam nesse tipo de problema. Os agentes de software possuem a capacidade de controlar o próprio comportamento e interagir com o ambiente. Podendo se adaptar ao contexto trocando mensagens com os veículos vizinhos e recuperando informações do ambiente. O objetivo desse trabalho é analisar o desempenho de agentes de software em redes veiculares construídas utilizando a abordagem ad hoc e híbrida. Para analisar o desempenho das duas abordagens foram realizadas simulações utilizando o software GRUBiX. Nas simulações os resultados demonstraram que os agentes apresentam desempenho melhores em redes híbridas do que em redes ad hoc. Então foi desenvolvido um protótipo baseado em kit com Arduíno e Zigbee para avaliar a viabilidade de uma rede veicular utilizando agentes de software para transportar dados. Os resultados demonstraram a viabilidade na utilização de agentes de software para o desenvolvimento de aplicações em redes veiculares construídas com o padrão Zigbee.}

\resumoingles{Vehicular Ad Hoc Networks - VANETs are networks with high mobility vehicles can communicate with each other or with infrastructure in roads, they can also carry data physically from one point to another. As there is a high mobility of vehicles in a city, the vehicular network needs an ability to rapidly adapt to the context. Software agents fit this kind of problem. Software agents have the ability to control their own behavior and interact with the environment. Being able to adapt to the context exchanging messages with neighboring vehicles and retrieving information from the environment. The aim of this study is to analyze the performance of software agents in vehicular networks built using ad hoc approach and hybrid. To analyze the performance of the two approaches simulations were performed using the software GRUBiX. In the simulations the results showed that agents have better performance in hybrid networks than in ad hoc networks. So we developed a prototype-based kit with Arduino and Zigbee to assess the feasibility of a network carrier using software agents to carry data. The results demonstrated the feasibility of using software agents for the development of applications in vehicular networks built with the standard Zigbee.}
...
\include{metodologia/simulacao}
\include{metodologia/prototipo}
