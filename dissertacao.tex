...
\title{Análise da utilização de agente de software em Redes Veiculares Híbridas utilizando ZigBee}
\date{2015}
...
\resumoingles{Vehicular Ad Hoc Networks - VANETs are networks with high mobility where vehicles can communicate with each other or with the road infrastructure , but can also carry data physically from one point to another. The aim of this study is to analyze the performance of software agents in vehicular networks built using ad hoc approach and hybrid . To analyze the performance of the two approaches simulations were performed using the software Grubi and developed a prototype -based kit with Arduino and Zigbee . The results showed that agents perform best in hybrid networks than in ad hoc networks.}
